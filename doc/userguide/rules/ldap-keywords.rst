...
and filter network packets based on LDAP result code.
...
.. table:: **Result code values for ldap.responses.result_code keyword**
...
keyword matches with any indice, but it is possible to specify a particular index for matching
and also use flags such as ``all`` and ``any``.
...
Example of a signatures that would alert if the packet has a ``success`` LDAP result code at any index :
...
Syntax::
 ...
 ldap.responses.message; content:"The Directory Server is shutting down";

...
use crate::detect::{
    ...
    DetectHelperMultiBufferMpmRegister, SIGMATCH_INFO_STICKY_BUFFER, SIGMATCH_NOOPT, DetectHelperGetMultiData,
    DetectBufferSetActiveList,
};
use crate::ldap::types::{LdapMessage, ProtocolOpCode, ProtocolOp};
    ...
    *buffer_len = 0;
    ...
    if let Some(request) = &tx.request {
        let str_buffer: &str = match &request.protocol_op {
            ProtocolOp::SearchRequest(req) => req.attributes[local_id as usize].0.as_str(),
            ProtocolOp::ModifyRequest(req) => req.changes[local_id as usize].modification.attr_type.0.as_str(),
            ProtocolOp::AddRequest(req) => req.attributes[local_id as usize].attr_type.0.as_str(),
...
pub unsafe extern "C" fn ScDetectLdapRegister() {
    ...
    let kw = SCSigTableElmt {
        ...
        url: b"/rules/ldap-keywords.html#ldap.request.attribute_type\0".as_ptr() as *const libc::c_char,
